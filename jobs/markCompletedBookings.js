...
async function markCompletedBookings() {
  ...
  for (const booking of bookings) {
    ...
    if (booking.guest?.email) {
      ...
      const propertyTitle = booking.property?.title || "the space";
      ...
      // Email 1 of 2: booking complete + deposit status.
      sendEmail({
        ...
        html: emailWrapper(`
          ...
          <p>Your booking at <strong>${propertyTitle}</strong> is now complete. We hope it went well!</p>
          ${hasDeposit ? `
          ...
          ` : ''}
          <p style="margin-bottom: 0;">If anything about your booking needs attention, get in touch from your VenCome dashboard and our team can help.</p>
        `),
      }).catch((err) => console.error(`Booking-complete email failed for booking ${booking._id}:`, err.message));

      // Email 2 of 2: review prompt, sent separately.
      sendEmail({
        to: booking.guest.email,
        subject: "How was your space?",
        html: emailWrapper(`
          <h2 style="color: #305CDE; text-align: center; margin-top: 0;">How was your stay?</h2>
          <p>Hi <strong>${guestName}</strong>,</p>
          <p>Now that your booking at <strong>${propertyTitle}</strong> is complete, we'd love to hear how it went. Your review helps other guests book with confidence.</p>
        ...
        `),
      }).catch((err) => console.error(`Review-prompt email failed for booking ${booking._id}:`, err.message));
    }
  }
